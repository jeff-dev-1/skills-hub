---
name: pptx-high-fidelity-reconstruction
description: Reconstruct high-fidelity editable PowerPoint decks from screenshots, slide images, PDFs, or visual references. Use when visual similarity matters but titles, body copy, KPIs, labels, and process text must remain editable; when image-only PPTX pages need to be converted to hybrid editable slides; or when prior native-shape reconstruction introduced double borders, crop boxes, spacing drift, or low-fidelity decorative redraws.
---

# High-Fidelity Editable PPTX Reconstruction
...
5. Extract icons as tight physical crops or use transparent SVG/PNG replacements. When the companion `icon-finder` skill is available, use it to find a style-consistent vector replacement and retain its license manifest. Prefer native PowerPoint shapes or SVG for rings, shields, targets, people, arrows, and other simple symbols. Insert each icon as an independent object. Do not rely on PowerPoint crop metadata unless the rendered preview proves it works.
